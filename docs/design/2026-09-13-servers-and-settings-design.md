...
A section that map does not name falls back to System rather than vanishing from a page that renders only what it recognises. Six things on the page are not configuration sections and so are not in the map: the server cards and the library map on Servers, and the General accordion of top-level scalars, the secrets accordion, the config-safety panel and the drift notice on System.
...
**8. A catch-up does not refuse a restart (section 4).** Section 4 lists a catch-up among the runs that refuse `POST /api/system/restart`. It does not. A catch-up's whole state is rows in the database and its drain resumes on the other side of the boot, so a restart during one interrupts nothing — while refusing would mean an operator with a day-long backlog could never apply a setting. Every other kind of run holds its work in this process's memory and does refuse. The third guard is the process-wide mode lock, which section 4 does not name at all: it is asked last, because it is the only one that must still be true at the instant of the exec, and it answers one of two sentences depending on whether a mode or an earlier restart holds it. One more correction belongs with these: section 3's "generated on the first boot that needs it" is, as shipped, the first *write* that needs it — reading never creates a key, so a boot whose state volume failed to mount cannot write a fresh key into the container filesystem and take the real one's place when the volume comes back.
