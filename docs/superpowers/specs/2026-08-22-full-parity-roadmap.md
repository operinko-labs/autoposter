...
| 171 | Search tail C: `decade`, and the `current_year` value grammar | 9b Task 6's per-family enumeration. `decade` — **1** of the 36 — plus a value grammar that is not an attribute: Kometa's `year: current_year`/`current_year-N` (`builder.py:4333-4348`) is a relative value on an INT, resolved at build time against the run's own clock, which is the same "resolved against the run, not against the parse" property `_Today` already has for the relative date windows. `decade` also has no `.not` (`no_not_mods`), so the modifier set is not the int set. Two things in one row because the second is worthless without the first | S–M — one row plus a resolved-at-build value type | parity-only | 101 |
