...
| 154 | `added` filter is timezone-dependent on the runner, not the Plex server | 9a Task 2 fix-round finding (`src/autoposter/collections/filter_values.py`, `filters.py::_as_calendar_date`): Plex sends `addedAt` as a unix epoch, and plexapi's `toDatetime` converts it with `datetime.fromtimestamp(value)` — no `tz`, because plexapi's own `DATETIME_TIMEZONE` is `None` — so the value lands as a naive datetime in the RUNNER's local clock, not the Plex server's, verified directly against the installed plexapi 4.18.2 (`plexapi/utils.py::_parseTimestamp`, `plexapi/video.py:44`). `_as_calendar_date` compares date-granularly, so the same item's `added` can fall on either side of an `added.after`/`added.before` boundary depending on which timezone the pass ran in. Open behavioral question, not yet decided: normalize to a fixed zone (UTC, or the Plex server's own — which would need a second value the listing doesn't carry) before the date comparison, or document the runner-dependence and accept it (today's posture, now stated correctly in the row note) | S — a decision, then either a one-line conversion or leaving the doc note as the answer | parity-only (not exercised: no `added.after`/`added.before` filter ships yet) | 96 |
