...
Fix round: `test_the_ancestor_walk_terminates_on_a_parent_id_cycle` (added `1bd8f24`) pins that a mutual `parent_id` cycle, built with a direct `UPDATE` after both rows exist, terminates the `_ancestors` walk rather than hanging the sweep, and that a cycle member is never misclassified as prunable.
...
Fix round (`5cf8c1f`): `test_a_connect_failure_refuses_without_leaking_the_server_address` pins that a `plex_factory` raised inside `job.run` (a refused connection, a rejected token, a plexapi `BadRequest`) becomes a class-name-only `PruneRefused` rather than leaking the server address, the same as a scan-time failure. `_ReupsertingPlex` (a `FakePlex` whose `exists_many` re-upserts the row mid-probe, from inside one `job.run`) backs two more: `test_an_applied_pass_counts_directories_off_what_it_actually_pruned` pins the directory recount excluding a row the pass skipped, and `test_a_skipped_rows_queued_job_is_not_dismissed` pins that a skipped row's queued job survives the dismissal pass.
