...
async def test_the_providers_step_never_serves_the_secret_it_minted(setup_client):
    """The minting step and the serving step are separate on purpose.

    The secret is minted by the first providers save and staged; the wizard
    shows it on the LAST pane, which the operator reaches several steps later.
    A save that answered with the value would make the reload in between lose
    it permanently -- the next save answers with nothing to mint -- and would
    persist at finish a webhook secret the operator never saw.
    """
    token = await _authenticate(setup_client)
    ...
    held = await setup_client.get("/api/setup/webhook-secret", headers=_headers(token))

    assert set(response.json()) == {"providers"}
    assert response.json()["providers"]["AUTOPOSTER_WEBHOOK_SECRET"] == setup_api.REDACTED
    # ...and it really was minted: the dedicated route has it.
    assert held.json()["webhook_secret"]
    assert held.json()["webhook_secret"] not in response.text


async def test_the_webhook_secret_is_served_exactly_once_and_never_logged(setup_client, caplog):
    ...
    plus a credential on the wire that came from a form. It is generated by the
    providers step, served by this route once, and answered as `null` by every
    later call -- the v1 shape, moved to a route of its own.
    """
    token = await _authenticate(setup_client)
    await setup_client.post("/api/setup/providers", json={"values": {}}, headers=_headers(token))

    with caplog.at_level(logging.DEBUG):
        first = await setup_client.get("/api/setup/webhook-secret", headers=_headers(token))
    second = await setup_client.get("/api/setup/webhook-secret", headers=_headers(token))
    progress = await setup_client.get("/api/setup/progress", headers=_headers(token))

    generated = first.json()["webhook_secret"]
    assert generated and len(generated) >= 32
    assert set(first.json()) == {"webhook_secret"}
    assert second.json() == {"webhook_secret": None}
    assert generated not in second.text
    assert generated not in progress.text
    assert generated not in "\n".join(r.getMessage() for r in caplog.records)


async def test_the_webhook_secret_route_answers_null_before_one_is_minted(setup_client):
    """Nothing is minted by asking: the providers step mints, this route
    serves. A wizard that reached the last pane without saving the provider
    form has no secret to be shown, and says so in the shape the pane already
    renders."""
    token = await _authenticate(setup_client)

    response = await setup_client.get("/api/setup/webhook-secret", headers=_headers(token))
    progress = await setup_client.get("/api/setup/progress", headers=_headers(token))

    assert response.json() == {"webhook_secret": None}
    assert progress.json()["providers"]["AUTOPOSTER_WEBHOOK_SECRET"] is None


async def test_the_staged_secret_survives_a_reload_that_dropped_the_token(setup_client):
    """What moving the reveal to the finish pane would otherwise have cost.

    The value is minted once and never again, so if the only copy were the
    providers response, a page reloaded between that save and the last pane --
    the Plex step is still to come -- would lose it for good and the finish
    step would persist a secret the operator never saw. Staged server-side and
    served by its own route, a reload costs the token and nothing else.
    """
    first_token = await _authenticate(setup_client)
    await setup_client.post(
        "/api/setup/providers", json={"values": {}}, headers=_headers(first_token)
    )

    # The reload: the token dies with the tab, the password is proved again.
    second_token = await _authenticate(setup_client)
    response = await setup_client.get("/api/setup/webhook-secret", headers=_headers(second_token))
...
async def test_an_empty_deployment_url_keeps_the_one_already_staged(setup_client, setup_state):
    ...
    from a step they stepped back into is to re-type an address the server
    already holds."""
    token = await _authenticate(setup_client)
    await setup_client.post(
        "/api/setup/public-url", json={"url": PUBLIC_URL}, headers=_headers(token)
    ...
    response = await setup_client.post(
        "/api/setup/public-url", json={"url": "  "}, headers=_headers(token)
    )

    assert response.status_code == 200, response.text
    assert response.json() == {"ok": True}
    assert setup_state.public_url == PUBLIC_URL


async def test_an_empty_database_url_keeps_the_one_already_staged_without_a_probe(
    setup_client, setup_state, monkeypatch
):
    """The same rule at the other one-field step, and one thing more: an empty
    submit opens no outbound connection. There is nothing new to try."""
    probed: list[str] = []
    ...
    response = await setup_client.post(
        "/api/setup/database", json={"url": ""}, headers=_headers(token)
    )

    assert response.status_code == 200, response.text
    assert probed == [FAKE_DB_URL]
    assert setup_state.staged["AUTOPOSTER_DATABASE_URL"] == FAKE_DB_URL
    assert "row-121-db-secret" not in response.text
...
async def _complete_every_step(
    client, token, monkeypatch, database_url: str = FAKE_DB_URL
) -> None:
    """Every step but the last, with the values the finish tests assert on.

    ``database_url`` defaults to the unreachable one every step before finish
    is happy with -- the probe is patched out -- and is passed the real one by
    the tests that walk THROUGH finish, which now writes into the database it
    was given.
    """
    monkeypatch.setattr(setup_api, "database_answers", _answering(True))
    await client.post(
...
async def test_the_written_config_document_is_the_one_the_loader_would_find(
    setup_client, monkeypatch, database_url
):
    from autoposter.config import loader as loader_module
...
async def test_finish_writes_the_staged_credentials_into_the_store(
    ...
    assert "AUTOPOSTER_DATABASE_URL" not in stored
    ...
    # The key never leaves the state directory, and is what the row above was
    # encrypted with.
    assert secret_store.secret_key_path().parent == state_module.state_dir()
...
async def test_finish_stores_nothing_and_execs_nothing_when_the_write_is_refused(
    ...
    async def refusing_store(session, name, value):
        raise RuntimeError(f"the row for {value} was refused")
    ...
    for secret in (FAKE_PLEX_TOKEN, database_url, MASTER_PASSWORD):
...
async def test_finish_falls_back_to_the_state_file_before_the_first_migration(
    ...
    """The ordinary FIRST START. `boot` runs `alembic upgrade head` only on the
    configured side of its branch, so a deployment being set up for the first
    time has never migrated and the secrets table does not exist yet at this
    write -- and will not until the boot this step execs.

    Refusing there would make the wizard unusable on every genuinely fresh
    deployment, which is the one it exists for. The credentials go to the state
    file instead, where every one of them went before the store existed, and
    the log line names the NAMES so an operator can see which are on the volume
    rather than in the table."""
    token = await _authenticate(setup_client)
    await _complete_every_step(
        setup_client, token, monkeypatch, database_url=tableless_database_url
    )
    monkeypatch.setattr(setup_api.os, "execv", lambda path, argv: None)
...
async def test_finish_writes_no_state_document_when_the_deployment_has_one(
    setup_client, monkeypatch, tmp_path, database_url
):
