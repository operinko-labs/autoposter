...
def _parse_bare(value: str) -> ParsedSource:
    ...
    if "?" in value or "#" in value:
        ...
        # query and no fragment -- `_SEGMENT`, `_LIST_ID` and `_USER_ID` admit
        # neither character -- so nothing valid is refused here, and every URL
        # shape that legitimately carries one (`imdb.com/list/ls1?ref_=hm`) is
        # resolved by the `_HOSTS` table and never reaches this function. What
        # DID reach it was `abc/def?apikey=SECRET`, whose head is dotless and
        # carries no ':' or '@', so it passed the guard above and went to
