...
FILTER_ATTRIBUTES: tuple[FilterAttribute, ...] = (
    ...
    FilterAttribute(
        ...
        "operator needs: Kometa spells this attribute two different ways in "
        "two different files, and both are transcribed as written. The "
        "OVERLAY pack (`defaults/overlays/aspect.yml`) windows each nominal "
        "ratio with `.gt`/`.lt` and never the inclusive forms, so its bands "
        "are open at both ends; the COLLECTIONS pack "
        "(`defaults/both/aspect.yml`) writes a bare `aspect: <value>` through "
        "its `filter` template, which is exact EQUALITY on one of eight "
        "values. Either way a 1.90 aspect matches none of them -- neither "
        "form is corrected here. An item Plex has not "
