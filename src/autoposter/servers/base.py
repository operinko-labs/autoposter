...
class MediaServer(Protocol):
    ...
    async def library_names(self) -> set[str]:
        """The libraries this server carries, named the way the rest of the
        configuration names them (spec §1).

        Plex answers its own non-excluded section titles. Jellyfin answers
        its non-excluded virtual folders translated back through
        ``library_map``, so both servers speak one name space -- which is
        what makes "this library is absent on that server" a decidable
        question rather than a guess about two naming conventions.
        """
        ...
