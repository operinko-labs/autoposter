...
def _config_ready(request: Request) -> bool:
    """Whether the config step has been completed -- staged, or already on
    disk from a wizard that got as far as writing it."""
    return (
        request.app.state.setup.config_document is not None
        or state_config_path().is_file()
    )
...
async def set_provider_keys(body: ProvidersRequest, request: Request) -> dict:
    ...
    thereafter.
    """
    unknown = sorted(set(body.values) - set(_PROVIDER_ENV))
    if unknown:
        # Names, not values: an unknown key is the caller's own typo.
        raise HTTPException(
            status_code=400,
            detail=f"not a credential this service reads: {', '.join(unknown)}",
        )
...
async def stage_config_document(body: ConfigRequest, request: Request) -> dict:
    ...
    if scheme not in {"http", "https"} or not separator or not rest:
...
async def finish(request: Request) -> JSONResponse:
    ...
    async with state.lock:
        ...
        merge_secrets_file(state.staged)

    persisted = resolve_secret_values()
    if not boot.is_configured(persisted):
        raise HTTPException(
            status_code=400,
            detail=_unmet_step(persisted, state_config_path().is_file())
